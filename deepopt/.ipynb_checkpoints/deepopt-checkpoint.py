...
from collections.abc import Iterable     # changed here, earlier: from collections import Iterable
...
#    old_items = np.array(old_items)
    old_items = np.array(list(old_items))
#    print("old items : ", old_items, "\nold items shape : ", old_items.shape, "\nnew_item : ", new_item)    # Debugging
...
class DeepOptEpoch(object):
    ...
    def set_constraints(self, constraintfn):
        ...
        for x in self.X_samples:
            if constraintfn(**self.point_to_kwargs(x)):
                X_samples.append(x)
        self.X_samples = X_samplesX_samples
    ...
    def add_param(self, name, values):
        ...
        list_X_samples = [float(t[0]) for t in list(self.X_samples)]       # Added this
        if self.traces is not None:
            self.traces.write(json.dumps(dict(action="add_param",
                                    X_samples = list_X_samples,
