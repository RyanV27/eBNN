...
import chainer
import chainer_sequential.link
#import link
import chainer_sequential.function
#import function
#from .function import *        # Function, Activation
import chainer_sequential.binary_link
# import binary_link
#from .binary_link import *     # BinaryLink
import chainer_sequential.binary_function
#import binary_function
#from .binary_function import *    # BinaryFunction, BinaryActivation
import numpy as np
...
#        elif isinstance(layer, link.Link) or isinstance(layer, function.Function):
        elif isinstance(layer, chainer_sequential.link.Link) or isinstance(layer, chainer_sequential.function.Function):
            self._layers.append(layer)
#        elif isinstance(layer, function.Activation):
        elif isinstance(layer, chainer_sequential.function.Activation):
...
#        elif isinstance(layer, binary_link.BinaryLink) or isinstance(layer, binary_function.BinaryFunction):
        elif isinstance(layer, chainer_sequential.binary_link.BinaryLink) or isinstance(layer, chainer_sequential.binary_function.Function):
            self._layers.append(layer)
#        elif isinstance(layer, binary_function.BinaryActivation):
        elif isinstance(layer, chainer_sequential.BinaryActivation):
            self._layers.append(layer.to_function())
        else:
            raise Exception()

    def layer_from_dict(self, dict):
#        print("dict[\"_link\"] = ", dict["_link"])              # Added this for debugging
        if "_link" in dict:
#            if hasattr(link, dict["_link"]):
            if hasattr(chainer_sequential.link, dict["_link"]):
                args = self.dict_to_layer_init_args(dict)
#                return getattr(link, dict["_link"])(**args)
                return getattr(chainer_sequential.link, dict["_link"])(**args)
#            elif hasattr(binary_link, dict["_link"]):
            elif hasattr(chainer_sequential.binary_link, dict["_link"]):
                args = self.dict_to_layer_init_args(dict)
#                return getattr(binary_link, dict["_link"])(**args)
                return getattr(chainer_sequential.binary_link, dict["_link"])(**args)
        if "_function" in dict:
#            if hasattr(function, dict["_function"]):
            if hasattr(chainer_sequential.function, dict["_function"]):
                args = self.dict_to_layer_init_args(dict)
#                return getattr(function, dict["_function"])(**args)
                return getattr(chainer_sequential.function, dict["_function"])(**args)
#            elif hasattr(binary_function, dict["_function"]):
            elif hasattr(chainer_sequential.binary_function, dict["_function"]):
                args = self.dict_to_layer_init_args(dict)
#                return getattr(binary_function, dict["_function"])(**args)
                return getattr(chainer_sequential.binary_function, dict["_function"])(**args)
        raise Exception()
